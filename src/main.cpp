    ...
    int main() {
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            if (show_chat_window) {
                ...
                ImGui::BeginChild("ChatMessages", ImVec2(0,360), true , ImGuiWindowFlags_AlwaysHorizontalScrollbar);
                
                    
                for (auto& msg : chat_message) {
                    bool isUserMessage = msg.find("You:") == 0; 
                    bool isBotMessage = msg.find("Bot: ") == 0;
                ...
                if (ImGui::Button("Send", buttonSize)) {
                    ...
                    if(strlen(bigText)> 0){
                        ...
                        try {
                            ...
                            Savechats(bigText);
                            std::string response = "Bot: " + r.get_response(bigText);
                            Savechats(response.c_str());
                            chat_message.push_back(response);
                            bigText[0] = '\0';
